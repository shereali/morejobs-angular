...
import {JobViewResumeComponent} from '../job-view-resume/job-view-resume.component';
...
export class ApplicantsComponent implements OnInit {
  ...
  loadData(params: ObjectMap, resetFilters = false): void {
    this.isLoading = true;
    this.spinner.show('applicant_table_spinner');
    const id = this.route.snapshot.params.id;
    this.jps.loadApplicantProcessApplicants(this.filterParams.formattedFilterParams(params), id)
      .then((res) => {
        if (res.success) {
      ...
      this.spinner.hide('applicant_table_spinner');
  ...
  handleResponse(res: any): void {
    ...
    this.applicants = res.data.records.data;
  ...
  openResumeDetailsModal(data: any): void {
    this.dialog.open(JobViewResumeComponent, {
